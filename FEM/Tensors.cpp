...
//   for (k=0; k<3; ++k)
//    for (l=0; l<3; ++l)
//     result_[i][j][k][l] = Data[i]*o_[j][k][l];
// return result_;
//}
////------------------------------------------------------------------------------
...
//     result_[i][j][k] = 0.l;
//     for (l=0; l<3; ++l)
//        result_[i][j][k] += Data[l] * o_[l][i][j][k];
//    }
// return result_;
//}
////------------------------------------------------------------------------------
...
////==========================Tensor2s=======================================
////------------------------------------------------------------------------------
//Tensor2s::Tensor2s (const Tensor1s& col1_, const Tensor1s& col2_, const Tensor1s& col3_)
//{
// __SET_ROWS;
// for (small_t i=0; i<3; ++i)
//    {
...
//{
// const real_t* po = o_.Data;
// for (real_t* pd=Data; pd<Data+9; ++pd, ++po)
//        *pd -= *po;
// return *this;
//}
////------------------------------------------------------------------------------
...
//{
// real_t *p0 = *Rows, *p1 = Rows[1], *p2 = Rows[2];
// const real_t *psrc0 = row1_.Data, *psrc1 = row2_.Data, *psrc2 = row3_.Data;
...
//{
// real_t *p0 = *Rows, *p1 = Rows[1], *p2 = Rows[2];
// const real_t *psrc0 = col1_.Data, *psrc1 = col2_.Data, *psrc2 = col3_.Data;
...
// det1 = 1.l/det1;
// real_t tmp[9] = {
//                  (Data[4]*Data[8] - Data[5]*Data[7]) * det1,
...
// const real_t i1=I1(),i2=I2(),i3=I3(), i1_3=i1/3.l,
//              p = i2-i1*i1_3,   q = i1_3*(i2 - 2.l*i1_3*i1_3) - i3;//coefs of reduced eqn
// Assert(LE0(p), "Eigen values with Im!=0 in Tensor2s::EigenValues(real_t[])");
// const real_t ro = sqrt(-p/3.l);//auxiliary param
// if (NE0(ro))
//   {
//    const real_t fi   = acos(-q/(2.l*ro*ro*ro))/3.l,//auxiliary param
//                 pi23 = acos(-0.5l);               //=2Pi/3
//    eigVals_[0] = 2.l*ro*cos(fi)         + i1_3;
//    eigVals_[1] = 2.l*ro*cos(fi+pi23)    + i1_3;
//    eigVals_[2] = 2.l*ro*cos(fi+pi23*2.) + i1_3;
//   }
// else
//    eigVals_[0] = eigVals_[1] = eigVals_[2] = (sign(pow(fabs(q),real_t(1.l/3.l)),-q) + i1_3);
//}
////------------------------------------------------------------------------------
//struct less_in_abs {bool operator()(const real_t& a, const real_t& b) const {return fabs(a)<fabs(b);}};
////------------------------------------------------------------------------------
//void Tensor2s::MaxAbsIndex(small_t& row_num_, small_t& col_num_) const
//{
// const small_t lin_num = max_element(Data,Data+9,less_in_abs()) - Data;
// row_num_ = lin_num / 3;
// col_num_ = lin_num % 3;
//}
////------------------------------------------------------------------------------
...
//{
//#ifdef STRONGCHECK
// Assert(begPos_>0 && begPos_<=3, "invalid position of eigenvector in Tensor2s::EigenVectors");
//#endif
// Tensor2s singular(*this);   real_t tmp;
// small_t i=0, j, i1, i2, j1, j2, multiplicity=1;   const small_t posOfCurrent=begPos_-1;
// for (   ; i<3; ++i)
//      singular[i][i] -= eigVal_;//i.e. singular = *this - eigval*E
//#ifdef STRONGCHECK
// Assert(EQ0(singular.Det(),PRECISION*50.l), tMessage("value expected to be eigen is not eigen in Tensor2s::EigenVectors\ndet(A-lambdaE) = ")<<singular.Det() << " (abs must be <= "<<PRECISION*20.l<<')');
//#endif
//// Index index;
// for (i=0; i<3; ++i)
//  for (j=0; j<3; ++j)
...
//    if (NE0(tmp))//If exist nonzero minor of 2nd order
//      {          //then rang of singular equals 2 and current eigen value is 1-fold:
//       i1 = i==0?1:0; i2 = 3-i-i1;
//       j1 = j==0?1:0; j2 = 3-j-j1;
//       eigVects_[j ][posOfCurrent] = tmp;
//       eigVects_[j1][posOfCurrent] = singular[i1][j2]*singular[i2][j]
//                                   - singular[i2][j2]*singular[i1][j];
//       eigVects_[j2][posOfCurrent] = singular[i2][j1]*singular[i1][j]
//                                   - singular[i1][j1]*singular[i2][j];
//       goto normalizing;
//      }
//   }
//#ifdef STRONGCHECK
...
// MaxAbsIndex(i,j);  j1 = j==0?1:0; j2 = 3-j-j1;
// eigVects_[j1][posOfCurrent] = eigVects_[j2][posOfCurrent+1] = 1.l;
// eigVects_[j2][posOfCurrent] = eigVects_[j1][posOfCurrent+1] = 0.l;
// if (NE0(singular[i][j]))
//    {
...
//#endif
//     eigVects_[j][0] = eigVects_[j][1] = eigVects_[j1][2] =
//                                         eigVects_[j2][2] = 0.l;
//     eigVects_[j][2] = 1.l;
//     return 3;
//    }
...
//             return;
// small_t next_eig_val_no = EQ(eigvals_[0],eigvals_[1]) ? 2 : 1;
...
// Assert(next_eig_val_no<2 || eig_vects_count==3, "eigen vectors are not correspond to eigen values in Tensor2s::Eigen");
//#endif
// if (eig_vects_count<3)
...
// for (small_t i=0; i<3; ++i)
//  {
//   rows_i0 = Rows[i][0];         rows_i1 = Rows[i][1];
...
//  {
//   rows_0j = Rows[0][j];         rows_1j = Rows[1][j];
...
//   for (k=0; k<3; ++k)
//    for (l=0; l<3; ++l)
//     result_[i][j][k][l] = Rows[i][j] * o_[k][l];
// return result_;
//}
////------------------------------------------------------------------------------
...
//{
//#ifdef STRONGCHECK
// Assert(index1No_!=index2No_ && index1No_>0 && index1No_<=4 && index2No_>0 && index2No_<=4, "invalid index # in Tensor2s::Dot2Multiply(SymmetricTensor4s&,small_t,small_t)");
//#endif
// Tensor4::tIndex ijkl;    const Tensor2s copyOfThis(*this);
// const small_t freeIndex1No = index1No_+index2No_==3?3:(index1No_==1||index2No_==1?2:1),
...
//       for (k=0; k<3; ++k)
//          for (l=0; l<3; ++l)
//             Rows[i][j] += copyOfThis[k][l] * o_[l][k][i][j];
//      }
// return *this;
//}
////------------------------------------------------------------------------------
...
//       for (k=0; k<3; ++k)
//          for (l=0; l<3; ++l)
//             Rows[i][j] += o_[i][j][k][l] * copyOfThis[l][k];
//      }
// return *this;
//}
////------------------------------------------------------------------------------
...
// Tensor2s currentEigDiade, result(0.l);
...
//          currentEigDiade *= tmp;//Direct product of i-th eigenvector to i-th eigenvector of the mutual basis is multiplied by the function of i-th eigenvalue
//          result += currentEigDiade;
//         }
//    }
...
//        for (l=0; l<3; ++l) result[l][l] += tmp;
//       }//result = E*f(fold_eigval) + (f(not_fold_eigval)-f(fold_eigval))*eigvect_diade_of_not_fold_eigval
//     else//if 3-fold eigen value
...
//{
// const real_t* po = o_.Data;
// for (real_t* pd=Data; pd<Data+27; ++pd, ++po)
//        *pd -= *po;
// return *this;
//}
////------------------------------------------------------------------------------
...
//{
//#ifdef STRONGCHECK
// Assert(n1_>0 && n2_>0 && n1_<=3 && n2_<=3 && n1_!=n2_, "invalid number of index in Tensor3s::Contraction");
//#endif
// const small_t k=6-n1_-n2_;  tIndex index;
// for (small_t i=0,j; i<3; ++i)
//    {
//     result_[i] = 0.l;
//     index[k] = i;
...
//   for (k=0; k<3; ++k)
//    for (l=0; l<3; ++l)
//     result_[i][j][k][l] = (*Rows[i])[j][k] * o_[l];
// return result_;
//}
////------------------------------------------------------------------------------
...
//  for (j=0; j<3; ++j)
//  {
//   data_ij0 = (*Rows[i])[j][0];   data_ij1 = (*Rows[i])[j][1];
//   (*Rows[i])[j][0] = data_ij0 * o_[0][0] + data_ij1 * o_[1][0] + (*Rows[i])[j][2] * o_[2][0];
//   (*Rows[i])[j][1] = data_ij0 * o_[0][1] + data_ij1 * o_[1][1] + (*Rows[i])[j][2] * o_[2][1];
//   (*Rows[i])[j][2] = data_ij0 * o_[0][2] + data_ij1 * o_[1][2] + (*Rows[i])[j][2] * o_[2][2];
//  }
// return *this;
//}
////------------------------------------------------------------------------------
...
//  for (k=0; k<3; ++k)
//  {
//   data_0jk = (*Rows[0])[j][k];   data_1jk = (*Rows[1])[j][k];
//   (*Rows[0])[j][k] = o_[0][0] * data_0jk + o_[0][1] * data_1jk + o_[0][2] * (*Rows[2])[j][k];
//   (*Rows[1])[j][k] = o_[1][0] * data_0jk + o_[1][1] * data_1jk + o_[1][2] * (*Rows[2])[j][k];
//   (*Rows[2])[j][k] = o_[2][0] * data_0jk + o_[2][1] * data_1jk + o_[2][2] * (*Rows[2])[j][k];
//  }
// return *this;
//}
////------------------------------------------------------------------------------
...
//     for (k=0; k<3; ++k)
//      for (l=0; l<3; ++l)
//       result_[i][j] += (*Rows[i])[k][l] * o_[l][k][j];
//    }
// return result_;
//}
////------------------------------------------------------------------------------
...
//      result_[i][j][k][l] = 0.l;
//      for (m=0; m<3; ++m)
//        result_[i][j][k][l] += (*Rows[i])[j][m] * o_[m][k][l];
//     }
// return result_;
//}
////------------------------------------------------------------------------------
...
//     for (k=0; k<3; ++k)
//      for (l=0; l<3; ++l)
//       result_[i] += (*Rows[j])[k][l] * o_[l][k][j][i];
//   }
// return result_;
//}
////------------------------------------------------------------------------------
...
//       for (l=0; l<k; ++l)
//         (*Rows[i])[j][k][l] = (*Rows[i])[j][l][k] = (*Rows[j])[i][k][l] = (*Rows[j])[i][l][k] = o_[tIndex(i,j,k,l)];
//      }
//  }
// return *this;
//}
...
//{
// const real_t* po = o_.Data;
// for (real_t* pd=Data; pd<Data+81; ++pd, ++po)
//        *pd -= *po;
// return *this;
//}
////------------------------------------------------------------------------------
...
//#endif
// Assign0();
// small_t i=0, j;     const small_t &k = (kind_==1)? i : j,
//                                   &l = (kind_==2)? i : j,
//                                   &m = (kind_==3)? i : j;
// for (   ; i<3; ++i)
//  for (j=0; j<3; ++j)
...
//{
//#ifdef STRONGCHECK
// Assert(n1_>0 && n2_>0 && n1_<=4 && n2_<=4 && n1_!=n2_, "invalid number of index in Tensor4s::Contraction");
//#endif
// small_t k1=10-n1_-n2_, k2=k1;  tIndex index4;
// if      (k1 > 5)
//                 {k2 = 4;  k1 -= k2;}
// else if (k1 < 5)
//                 {k1 = 1;  k2 -= k1;}
...
//     result_[i][j][k] = 0.l;
//     for (l=0; l<3; ++l)
//      result_[i][j][k] += (*Rows[i])[j][k][l] * o_[l];
//    }
// return result_;
//}
////------------------------------------------------------------------------------
...
//{
// const Tensor4s copyOfThis(*this);      const Tensor3s  levyChivita(eUnit);
// for (small_t i=0,j,k,l,m,n; i<3; ++i)
//  for (j=0; j<3; ++j)
//   for (k=0; k<3; ++k)
//    for (l=0; l<3; ++l)
...
//     for (m=0; m<3; ++m)
//      for (n=0; n<3; ++n)
//        (*Rows[i])[j][k][l] += copyOfThis[i][j][k][m] * o_[n] * levyChivita[m][n][l];
//    }
// return *this;
//}
////------------------------------------------------------------------------------
...
//     for (m=0; m<3; ++m)
//      for (n=0; n<3; ++n)
//        (*Rows[i])[j][k][l] += o_[m] * copyOfThis[n][j][k][l] * levyChivita[m][n][i];
//    }
// return *this;
//}
////------------------------------------------------------------------------------
...
//{
//#ifdef STRONGCHECK
// Assert(ownIndexNo_>0 && ownIndexNo_<=4 && facIndexNo_>0 && facIndexNo_<=2, "invalid index # in Tensor4s::DotMultiply(Tensor4s&,small_t,small_t)");
//#endif
// real_t data_ijk0, data_ijk1;  Tensor2s::tIndex index2;  tIndex index4;
...
//   for (k=0; k<3; ++k)
//   {
//    data_ijk0 = (*Rows[i])[j][k][0];   data_ijk1 = (*Rows[i])[j][k][1];
//    (*Rows[i])[j][k][0] = data_ijk0*o_[0][0] + data_ijk1*o_[1][0] + (*Rows[i])[j][k][2]*o_[2][0];
//    (*Rows[i])[j][k][1] = data_ijk0*o_[0][1] + data_ijk1*o_[1][1] + (*Rows[i])[j][k][2]*o_[2][1];
//    (*Rows[i])[j][k][2] = data_ijk0*o_[0][2] + data_ijk1*o_[1][2] + (*Rows[i])[j][k][2]*o_[2][2];
//   }
// return *this;
//}
////------------------------------------------------------------------------------
...
//   for (l=0; l<3; ++l)
//   {
//    data_0jkl = (*Rows[0])[j][k][l];   data_1jkl = (*Rows[1])[j][k][l];
//    (*Rows[0])[j][k][l] = o_[0][0]*data_0jkl + o_[0][1]*data_1jkl + o_[0][2]*(*Rows[2])[j][k][l];
//    (*Rows[1])[j][k][l] = o_[1][0]*data_0jkl + o_[1][1]*data_1jkl + o_[1][2]*(*Rows[2])[j][k][l];
//    (*Rows[2])[j][k][l] = o_[2][0]*data_0jkl + o_[2][1]*data_1jkl + o_[2][2]*(*Rows[2])[j][k][l];
//   }
// return *this;
//}
////------------------------------------------------------------------------------
...
//    for (k=0; k<3; ++k)
//     for (l=0; l<3; ++l)
//      result_[i] += (*Rows[i])[j][k][l] * o_[l][k][j];
//  }
// return result_;
//}
////------------------------------------------------------------------------------
...
//   for (k=0; k<3; ++k)
//    for (l=0; l<3; ++l)
//     result += (*Rows[i])[j][k][l] * o_[l][k][j][i];
// return result;
//}
////------------------------------------------------------------------------------
...
//        for (l=0; l<3; ++l)
//         for (m=0; m<3; ++m)
//          result_[i][j] += (*Rows[i])[k][l][m] * o_[m][l][k][j];
//      }
// return result_;
//}
////------------------------------------------------------------------------------
...
//       for (m=0; m<3; ++m)
//        for (n=0; n<3; ++n)
//         (*Rows[i])[j][k][l] += tmp[m][n] * o_[n][m][k][l];
//      }
//   }
// return *this;
//}
...
//       for (m=0; m<3; ++m)
//        for (n=0; n<3; ++n)
//         (*Rows[i])[j][k][l] += o_[i][j][m][n] * tmp[n][m];
//      }
//   }
// return *this;
//}
