...
{
#ifdef STRONGCHECK
	std::vector<const tNode*>::iterator ppnode =
		std::find(pNodes.begin(), pNodes.end(), (const tNode*)NULL);
...
{
	tMapRotToLoc::iterator p = RotToLoc_DB.find(
...
const Tensor2s& t1D_FE::JacobyMatrix_l2g(const Tensor1s& lc_) const
{
	...
	if (p == JacobyMatrix_DB.end())
	{
		...
		pair<map<real_t, Tensor2s>::iterator, bool> ins =
			JacobyMatrix_DB.insert(map<real_t, Tensor2s>::value_type(lc_(1), Tensor2s()));
		Assert(
			ins.second,
...
real_t t1D_FE::Jacobian_l2g(const Tensor1s& lc_) const
{
	...
	{
#ifdef STRONGCHECK
		pair<map<real_t, real_t>::iterator, bool> ins = Jacobian_DB.insert(
		...
		p = ins.first;
#else
		p = Jacobian_DB.insert(map<real_t, real_t>::value_type(lc_(1), JacobyMatrix_l2g(lc_).Det()))
				.first;
#endif
	}
	return p->second;
...
real_t t2D_FE::Jacobian_l2g(const Tensor1s& lc_) const
{
	...
	if (p == Jacobian_DB.end())
	{
		...
		pair<map<pair<real_t, real_t>, real_t>::iterator, bool> ins =
			Jacobian_DB.insert(map<pair<real_t, real_t>, real_t>::value_type(
...
real_t t3D_FE::Jacobian_l2g(const Tensor1s& lc_) const
{
	...
	if (p == Jacobian_DB.end())
	{
		...
		pair<map<pair<pair<real_t, real_t>, real_t>, real_t>::iterator, bool> ins =
			Jacobian_DB.insert(map<pair<pair<real_t, real_t>, real_t>, real_t>::value_type(
...
const Tensor2s& t2D_FE::JacobyMatrix_l2g(const Tensor1s& lc_) const
{
	...
	{
#ifdef STRONGCHECK
		pair<map<pair<real_t, real_t>, Tensor2s>::iterator, bool> ins = JacobyMatrix_DB.insert(
...
const Tensor2s& t3D_FE::JacobyMatrix_l2g(const Tensor1s& lc_) const
{
	...
	if (p == JacobyMatrix_DB.end())
	{
		...
		pair<map<pair<pair<real_t, real_t>, real_t>, Tensor2s>::iterator, bool> ins =
			JacobyMatrix_DB.insert(map<pair<pair<real_t, real_t>, real_t>, Tensor2s>::value_type(
...
Tensor1s& tIsoRod2ConstSec::CoordShapeGrad(
	cardinal_t no_, const Tensor1s& lc_, Tensor1s& result_) const
{
...
//------------------------------------------------------------------------------
Tensor1s& tIsoRod2ConstSec::DisplShapeGrad(cardinal_t no_, const Tensor1s& lc_, Tensor1s& result_)
const
{
 return (*(pShapeFunGrads(no_)))(lc_,result_);
}*/
...
#ifdef STRONGCHECK
	Assert(
		lc_(1) >= -1.l && lc_(1) <= 1.l && lc_(2) >= -1.l && lc_(2) <= 1.l,
...
//------------------------------------------------------------------------------
Tensor1s& tIsoQuad4ConsThick::DisplShapeGrad(cardinal_t no_, const Tensor1s& lc_, Tensor1s& result_)
const
{
 return (*(pShapeFunGrads(no_)))(lc_,result_);
}*/
...
#ifdef STRONGCHECK
	Assert(
		lc_(1) >= -1.l && lc_(1) <= 1.l && lc_(2) >= -1.l && lc_(2) <= 1.l && lc_(3) >= -1.l &&
			lc_(3) <= 1.l,
		"value of local coord must be between -1 & 1");
#endif
