...
//
//   std::vector<const tNode*>::const_iterator p1stNode() const {return pNodes.begin();}//for use in T tIsoparametricFE::tArray<T>::operator()
//
//   static tFinitElement* NewFE (const std::string& kindName_) {return Factory.CallFunction(kindName_);}
// protected:
//   tFinitElement (): pMaterial(NULL), StrainLevel(0.) {}
//   tFinitElement (cardinal_t numberOfNodes_): pNodes(numberOfNodes_,NULL), pMaterial(NULL), StrainLevel(0.) {}
//   void DefineNextNode (const tNode&);
// public:
//   virtual ~tFinitElement() {}
//   virtual const std::string& Kind() const =0;
//   virtual tFinitElement& DefineNextNode (const tNode&, bool, bool) =0;
//   cardinal_t HowManyNodes() const {return pNodes.size();}
...
//   bool Has(const tNode& o_) const {return std::find(pNodes.begin(),pNodes.end(),&o_) > pNodes.end();}
//   tFinitElement& Link (const tMaterial&);
//   const tMaterial& Material() const {return *pMaterial;}
//   bool Has(const tMaterial& o_) const {return pMaterial == &o_;}
...
//   Tensor2s& JacobyMatrix_l2g (const Tensor1s&, Tensor2s&) const;  // be careful - do not call for 1D or 2D - FEs!!! (because singular)
// public:
...
//          real_t& operator()(const TArg& x_) const {return Result = FE.Jacobian_l2g(x_);}
//     };
//   //----
...
//        struct fIntegrand { virtual TRet& operator()(const TArg&) const =0; };
//   template <typename TArg>
//     struct fOne: public fIntegrand<TArg,real_t>
//      {
//         mutable real_t Result;
////         fOne(): Result(1.l) {}
//         real_t& operator()(const TArg&) const {return Result = 1.l;}
////         real_t operator()(const TArg&) const {return 1.l;}
//        template <typename TArg2>
//          real_t& operator()(const TArg& x_, const TArg2& y_) const {return Result = 1.;}  //need for tParallelepiped1
//      };
...
//         TRet& operator()(const TArg& x_) const {return Multiplicand(x_) *= Factor(x_);}
//      };
//   template <typename TArg, typename TRet, typename TMult, typename TFac>
//     static fMultWithAssgn<TArg,TRet,TMult,TFac> MultBy(TMult& m_,const TFac& f_) {return fMultWithAssgn<TArg,TRet,TMult,TFac>(m_,f_);}
//
...
//
////   virtual Tensor2s& Integrate (const fIntegrand<Tensor1s,Tensor2s>&,Tensor2s&) const =0;//for strain
//   virtual SymmetricTensor2s& Integrate (const fIntegrand<Tensor1s,SymmetricTensor2s>&,SymmetricTensor2s&) const =0;//for strain
//
//   //----
////   tFEsSetOfTensor2& Calculate(fIntegrand<tFEsSetOfTensor2>& fun_, tEvalMethod em_, tFEsSetOfTensor2& result_) const {return em_==atCentre? result_.Swap(fun_(Tensor1s(0.l))) : Integrate(fun_,result_) /= Volume();}
////   Tensor1& Displacement (const Tensor1&, Tensor1&) const;
////   Tensor2s& DisplacementGradient (const Tensor1s&, Tensor2s&) const;
////   tFEsTensor2& DisplacementGradient (const Tensor1s&, tFEsTensor2&) const;
//   virtual real_t Volume() const {const fOne<Tensor1s> one; return Integrate(one);}
...
// protected:
//   void DefineCoordShapeFunAndItsGrad (cardinal_t,real_t(*)(const Tensor1s&),Tensor1s&(*)(const Tensor1s&, Tensor1s&));
//   void DefineDisplShapeFunAndItsGrad (cardinal_t,real_t(*)(const Tensor1s&),Tensor1s&(*)(const Tensor1s&, Tensor1s&));
//   //===end of data for providing calls of shape functions and its gradients
//   tNonIsoparametricFE(cardinal_t coordNod_,cardinal_t displNod_): CoordApproxData(coordNod_),DisplApproxData(displNod_){}
// public:
//   virtual tFinitElement& DefineNextNode (const tNode&, bool, bool);
//   cardinal_t HowManyCoordApproxNodes() const {return CoordApproxData.size();}
//   cardinal_t HowManyDisplApproxNodes() const {return DisplApproxData.size();}
...
//   //===Data for providing calls of shape functions and its gradients:
// private:
//  template <typename tTYPE> class tArray: public std::vector<tTYPE>
//                          {
//                           public:
...
//// friend   real_t(*)(const Tensor1s&)          tArrayOfPtrsToShapeFunctions::operator()(const tFinitElement*,const tNode*) const;
//// friend Tensor1&(*)(const Tensor1s&,Tensor1&) tArrayOfPtrsToShapeFunGradients::operator()(const tFinitElement*,const tNode*) const;
//
////**************** ERROR in g++ 4.0.2 **********************
...
//   tIsoparametricFE() {}
// public:
//   virtual cardinal_t HowManyCoordApproxNodes() const {return HowManyNodes();}
//   virtual cardinal_t HowManyDisplApproxNodes() const {return HowManyNodes();}
//   virtual const tNode& CoordApproxNode (cardinal_t i_) const {return Node(i_);}
//   virtual const tNode& DisplApproxNode (cardinal_t i_) const {return Node(i_);}
//#ifdef STRONGCHECK
//   virtual tFinitElement& DefineNextNode (const tNode&, bool=true, bool=true);
//#else//ifndef STRONGCHECK
//   virtual tFinitElement& DefineNextNode (const tNode& node_, bool=true, bool=true) {tFinitElement::DefineNextNode(node_);return *this;}
//#endif//def STRONGCHECK
//   virtual real_t   DisplShapeFun(cardinal_t n_, const Tensor1s& c_) const {return CoordShapeFun(n_,c_);}
//   virtual Tensor1s& DisplShapeGrad(cardinal_t n_, const Tensor1s& c_, Tensor1s& r_) const {return CoordShapeGrad(n_,c_,r_);}
//   virtual real_t   DisplShapeFun(const tNode& n_, const Tensor1s& c_) const {return CoordShapeFun(n_,c_);}
//   virtual Tensor1s& DisplShapeGrad(const tNode& n_, const Tensor1s& c_, Tensor1s& r_) const {return CoordShapeGrad(n_,c_,r_);}
//};
////------------------------------------------------------------------------------
...
//        {return Integrator.ByPlusAssgn<TFunArg>(MultBy<TFunArg,TFunRet>(fun_,fJacobian_l2g(*this)),result_);
//        }//--------------
//   template <typename TFunArg, typename TFunRet, typename TFun>
//    TFunRet& Integrate_local (TFun& fun_, TFunRet& result_) const
//        {return Integrate1D_local<TFunArg>(MultBy<TFunArg,TFunRet>(fun_,fCrossSecArea(*this)),result_);
//        }//--------------
//   template <typename TFunArg, typename TFunRet, typename TFun>
...
// public:
//   //--------------
//   virtual Tensor2a& Integrate_local (const fIntegrand<Tensor1s,Tensor2a>& fun_, Tensor2a& result_) const
...
//        }//--------------
////   virtual Tensor2s& Integrate (const fIntegrand<Tensor1s,Tensor2s>& fun_, Tensor2s& result_) const//for strain
//   virtual SymmetricTensor2s& Integrate (const fIntegrand<Tensor1s,SymmetricTensor2s>& fun_, SymmetricTensor2s& result_) const//for strain
//        {return Integrate<Tensor1s>(fun_,result_);
//        }//--------------
//   //--------------
//
...
// public:
//   virtual cardinal_t SpaceDimension() const {return 1;}
//   virtual Tensor1s& GlobalCoord (const Tensor1s&, Tensor1s&) const;
// private:
...
//   t1D_FE& Set1dStress(bool setToStress_) {StateIs1dStress=setToStress_; return *this;}
//   bool Has1dStressState() const {return StateIs1dStress;}
//   virtual t1D_FE& SetCrosSecArea (real_t) =0;
//   virtual real_t CrosSecArea (real_t=0.l) const =0;
//           real_t CrosSecArea (const Tensor1s& lc_) const {return CrosSecArea(lc_(1));}
//   class fCrossSecArea
//     {
//      private:
//        mutable real_t Result;
...
//        template <typename TArg>
//          real_t& operator()(const TArg& x_) const {return Result = FE.CrosSecArea(x_);}
//     };
//   virtual real_t Length() const {real_t result(0.l); const fOne<Tensor1s> one; return Integrate1D<Tensor1s>(one,result);}
//
//   virtual const SymmetricTensor4s& MaterialElasTensor(const Tensor1s&) const;
//};
////------------------------------------------------------------------------------
...
//   //----
////   t2D_FE& Set2dStress(bool setToStress_) {StateIs2dStress=setToStress_; return *this;}
////   bool Has2dStressState() const {return StateIs2dStress;}
...
//        template <typename TArg>
//          real_t& operator()(const TArg& x_) const {return Result = FE.Thickness(x_(1),x_(2));}
//        template <typename TArg>
//          real_t& operator()(const TArg& x_, const TArg& y_) const {return Result = FE.Thickness(x_,y_);}
//     };
//   virtual real_t Area()  const {const fOne<Tensor1s> one; return Integrate2D(one);}
//   //----
//   virtual const SymmetricTensor4s& MaterialElasTensor(const Tensor1s&) const;
//};
////------------------------------------------------------------------------------
...
////     template<typename T> T& Integrate3D (const tFinitElement::fIntegrand<T>&, T&) const;//2nd parameter must be zero!!!
////     template<typename T> T& Integrate (const tFinitElement::fIntegrand<T>&, T&) const;//2nd parameter must be zero!!!
//   public:
//   virtual const Tensor2s& JacobyMatrix_l2g (const Tensor1s&) const;
//   virtual real_t Jacobian_l2g (const Tensor1s&) const;
//   virtual Tensor2s& TensorOfRotationToLocal(const Tensor1s&, Tensor2s&) const;
...
//
//     virtual cardinal_t SpaceDimension() const {return 3;}
//};
////------------------------------------------------------------------------------
...
//   //--------------
//   template<typename TFunArg, typename TFunRet, typename TFun>
//   TFunRet& Integrate2D_local (TFun fun_, TFunRet& result_) const//2nd parameter isn't assigned by 0!!!
//        {
//         return Integrator.ByPlusAssgn<TFunArg>(fun_,result_);
//        }
//   //--------------
...
//                TFunRet& Integrate_local (TFun& fun_, TFunRet& result_) const
//                  {
//                    return Integrate2D_local<TFunArg>(MultBy<TFunArg,TFunRet>(fun_,fThickness(*this)),result_);
////                    return Integrate2D_local<TFunArg>(fun_,result_); //isn't right, for experiment only
//                  }
//   template <typename TFunArg, typename TFunRet, typename TFun>
//                TFunRet& Integrate (TFun& fun_, TFunRet& result_) const
//                  {
...
//       {
//        real_t result(0.l); const fOne<Tensor1s> one; return Integrate2D_local<Tensor1s>(one,result);
//       }
//   virtual real_t Integrate2D (const fIntegrand<Tensor1s,real_t>& fun_) const
//       {
//        real_t result(0.l); const fOne<Tensor1s> one; return Integrate2D<Tensor1s>(one,result);
//       }
//   //--------------
//   virtual cardinal_t SpaceDimension() const {return 2;}
//
//};
////------------------------------------------------------------------------------
...
//      {public:   tPtrShapeFunArray(): tIsoparametricFE::tArrayOfPtrsToShapeFunctions(2)
//                     {operator[](0)=ShapeFun1; operator[](1)=ShapeFun2;}
//      };
...
//      {public:   tPtrShapeFunGradArray(): tIsoparametricFE::tArrayOfPtrsToShapeFunGradients(2)
//                     {operator[](0)=ShapeGrad1; operator[](1)=ShapeGrad2;}
//      };
...
////   virtual real_t   DisplShapeFun             (const tNode&, const Tensor1s&) const;
////   virtual Tensor1s& DisplShapeGrad(const tNode&, const Tensor1s&, Tensor1s&) const;
//   static tFinitElement* NewFE() {return new tIsoRod2ConstSec();}
//
////   virtual Tensor1s& Tangent(const Tensor1s&, Tensor1s& result_) const {return (result_=Node(2).Coord();result_-=Node(1).Coord());}
////   virtual real_t Length() const {return (Tensor1s d=Node(2).Coord();d-=Node(1).Coord()).Length();}
//   virtual t1D_FE& SetCrosSecArea (real_t newval_) {Area=newval_; return *this;}
//   virtual real_t CrosSecArea (real_t=0.l) const {return Area;}
...
//   class tPtrShapeFunArray: public tIsoparametricFE::tArrayOfPtrsToShapeFunctions
//      {public:   tPtrShapeFunArray();
//      };
//   static const tPtrShapeFunArray pShapeFunctions;
// friend class tPtrShapeFunArray;
//   class tPtrShapeFunGradArray: public tIsoparametricFE::tArrayOfPtrsToShapeFunGradients
//      {public:   tPtrShapeFunGradArray();
//      };
//   static const tPtrShapeFunGradArray pShapeFunGrads;
// friend class tPtrShapeFunGradArray;
//   //----
...
////   virtual Tensor1s& DisplShapeGrad(const tNode&, const Tensor1s&, Tensor1s&) const;
//
//   static tFinitElement* NewFE() {return new tIsoQuad4ConsThick();}
//
//   virtual const std::string& Kind() const {return KindName;}
////   virtual real_t Volume() const {return Area() * ThicknessValue;}
//   virtual t2D_FE& SetThickness(real_t newval_) {ThicknessValue=newval_; return *this;}
//   virtual real_t Thickness(real_t=0.l, real_t=0.l) const {return ThicknessValue;}
//};
////------------------------------------------------------------------------------
...
//   //--------------
//   template<typename TFunArg, typename TFunRet, typename TFun>
//   TFunRet& Integrate3D_local (TFun fun_, TFunRet& result_) const//2nd parameter isn't assigned by 0!!!
//        {
//         return Integrator.ByPlusAssgn<TFunArg>(fun_,result_);
//        }
//   //--------------
...
////        fOne() {}
//        template <typename TArg>
//          real_t& operator()(const TArg& x_) const {return Result = 1.;}
//        template <typename TArg>
//          real_t& operator()(const TArg& x_, const TArg& y_) const {return Result = 1.;}
//     }; */
//   template <typename TFunArg, typename TFunRet, typename TFun>
//                TFunRet& Integrate_local (TFun& fun_, TFunRet& result_) const
//                  {
//                    return Integrate3D_local<TFunArg>(MultBy<TFunArg,TFunRet>(fun_,fOne<Tensor1s>()),result_);
...
//
//   virtual const std::string& Kind() const {return KindName;}
//   static tFinitElement* NewFE() {return new tIsoParallelepiped8();}
//};
////------------------------------------------------------------------------------
////--------------------------tFinitElement---------------------------------------
////------------------------------------------------------------------------------
...
////--------------------------tNonIsoparametricFE---------------------------------
////------------------------------------------------------------------------------
//inline void tNonIsoparametricFE::DefineCoordShapeFunAndItsGrad (cardinal_t number_,
//                                                   real_t(*pfun_)(const Tensor1s&),
//                                                   Tensor1s&(*pgrad_)(const Tensor1s&, Tensor1s&)
//                                                   )
//{
//#ifdef STRONGCHECK
// Assert(number_>0 && number_<=CoordApproxData.size(), "invalid number of shape function in tFinitElement::DefineCoordShapeFunAndItsGrad");
// Assert(CoordApproxData[number_-1].pFunction     == NULL, "shape function is already defined in tFinitElement::DefineCoordShapeFunAndItsGrad");
// Assert(CoordApproxData[number_-1].pFunctionGrad == NULL, "shape gradient is already defined in tFinitElement::DefineCoordShapeFunAndItsGrad");
//#endif
// CoordApproxData[--number_].pFunction     = pfun_;
// CoordApproxData[  number_].pFunctionGrad = pgrad_;
//}
////------------------------------------------------------------------------------
//inline void tNonIsoparametricFE::DefineDisplShapeFunAndItsGrad (cardinal_t number_,
//                                                   real_t(*pfun_)(const Tensor1s&),
//                                                   Tensor1s&(*pgrad_)(const Tensor1s&, Tensor1s&)
//                                                   )
//{
//#ifdef STRONGCHECK
// Assert(number_>0 && number_<=DisplApproxData.size(), "invalid number of shape function in tFinitElement::DefineDisplShapeFunAndItsGrad");
// Assert(DisplApproxData[number_-1].pFunction     == NULL, "shape function is already defined in tFinitElement::DefineDisplShapeFunAndItsGrad");
// Assert(DisplApproxData[number_-1].pFunctionGrad == NULL, "shape gradient is already defined in tFinitElement::DefineDisplShapeFunAndItsGrad");
//#endif
...
////       p = std::find_if(CoordApproxData.begin(),CoordApproxData.end(),tNodeAnd2FunsPtrs::Node_address_is(&node_));
// Assert(std::find_if(CoordApproxData.begin(),CoordApproxData.end(),tNodeAnd2FunsPtrs::Node_address_is(&node_))<CoordApproxData.end(), "There are no shape functions corresponding to the node in tFinitElement::CoordShapeFun");
//#endif
//// return (*(p->pFunction))(lc_);
// return (*(std::find_if(CoordApproxData.begin(),CoordApproxData.end(),tNodeAnd2FunsPtrs::Node_address_is(&node_))->pFunction))(lc_);
//}
////------------------------------------------------------------------------------
//inline Tensor1s& tNonIsoparametricFE::CoordShapeGrad (const tNode& node_, const Tensor1s& lc_, Tensor1s& result_) const
//{
//#ifdef STRONGCHECK
//// std::vector<tNodeAnd2FunsPtrs>::const_iterator
// Assert(std::find_if(CoordApproxData.begin(),CoordApproxData.end(),tNodeAnd2FunsPtrs::Node_address_is(&node_))<CoordApproxData.end(), "There are no shape function grads corresponding to the node in tFinitElement::CoordShapeGrad");
//#endif
//// return (*(p->pFunctionGrad))(lc_,result_);
// return (*(std::find_if(CoordApproxData.begin(),CoordApproxData.end(),tNodeAnd2FunsPtrs::Node_address_is(&node_))->pFunctionGrad))(lc_,result_);
//}
////------------------------------------------------------------------------------
...
//#ifdef STRONGCHECK
//// std::vector<tNodeAnd2FunsPtrs>::const_iterator
// Assert(std::find_if(DisplApproxData.begin(),DisplApproxData.end(),tNodeAnd2FunsPtrs::Node_address_is(&node_))<DisplApproxData.end(), "There are no shape functions corresponding to the node in tFinitElement::DisplShapeFun");
//#endif
//// return (*(p->pFunction))(lc_);
// return (*(std::find_if(DisplApproxData.begin(),DisplApproxData.end(),tNodeAnd2FunsPtrs::Node_address_is(&node_))->pFunction))(lc_);
//}
////------------------------------------------------------------------------------
//inline Tensor1s& tNonIsoparametricFE::DisplShapeGrad (const tNode& node_, const Tensor1s& lc_, Tensor1s& result_) const
//{
//#ifdef STRONGCHECK
//// std::vector<tNodeAnd2FunsPtrs>::const_iterator
// Assert(std::find_if(DisplApproxData.begin(),DisplApproxData.end(),tNodeAnd2FunsPtrs::Node_address_is(&node_))<DisplApproxData.end(), "There are no shape function grads corresponding to the node in tFinitElement::DisplShapeGrad");
//#endif
//// return (*(p->pFunctionGrad))(lc_,result_);
// return (*(std::find_if(DisplApproxData.begin(),DisplApproxData.end(),tNodeAnd2FunsPtrs::Node_address_is(&node_))->pFunctionGrad))(lc_,result_);
//}
////------------------------------------------------------------------------------
//#endif // FElemH
