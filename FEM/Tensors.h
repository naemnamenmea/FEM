...
public:
	...
	{
		return Tensor1s(-Data[0], -Data[1], -Data[2]);
	}
	Tensor1s operator+(const Tensor1s& o_) const
	{
		return Tensor1s(Data[0] + o_[0], Data[1] + o_[1], Data[2] + o_[2]);
	}
	Tensor1s operator-(const Tensor1s& o_) const
	{
		return Tensor1s(Data[0] - o_[0], Data[1] - o_[1], Data[2] - o_[2]);
	...
	{
		return Tensor1s(Data[0] * v_, Data[1] * v_, Data[2] * v_);
	}
	bool operator==(const Tensor1s& o_) const
	{
		return EQ(Data[0], o_[0]) && EQ(Data[1], o_[1]) && EQ(Data[2], o_[2]);
	...
	Tensor1s& Assign1(small_t i_, real_t v_ = 1.l)
	{
		__Assign(0.l);
		Data[--i_] = v_;
		return *this;
...
private:
	Tensor1s* __pData()
	{
		return is0() ? (pData = new Tensor1s) : pData;
	}
	bool __AdjustAdd(const Tensor1a& o_)
	{
		return is0() ? (o_.is0() ? false : (pData = new Tensor1s(*o_.pData), false))
					 : (o_.is0() ? false : true);
	}
	template <typename T>
	...
	const real_t operator[](small_t i_) const
	{
		return is0() ? 0.l : pData->operator[](i_);
	}
...
public:
	...
	Tensor1a& Assign1(small_t i_, real_t v_ = 1.l)
	{
		if (is0())
			pData = new Tensor1s(true, i_, v_);
		else
			pData->Assign1(i_, v_);
		return *this;
	}
	...
	{
		if (is0())
			pData = new Tensor1s(0.l);
		pData->Data[--i_] = v_;
		return *this;
	}
	Tensor1a& Assign(real_t x_, real_t y_, real_t z_)
	{
		is0() ? pData = new Tensor1s(x_, y_, z_) : &pData->Assign(x_, y_, z_);
		return *this;
	}
	...
	{
		return is0() || o_.is0() ? 0.l : pData->DotProduct(*o_.pData);
	}
...
class RetTensor1
{
	...
	RetTensor1(Tensor1a& o_) : pData(o_.pData)
	{
		o_.pData = NULL;
	}
...
public:
	...
	Tensor2s(tensor_kind_t, real_t v_ = 0.l)
	{
		__SET_ROWS;
		Assign1(v_);
	}
	...
	{
		std::for_each(Data, Data + 9, fMilt<real_t, real_t>(v_));
		return *this;
	}
	...
	Tensor2s& Negate()
	{
		std::transform(Data, Data + 9, Data, std::negate<real_t>());
		return *this;
	}
	Tensor2s& Transpose();
	...
	Tensor2s& Assign1(real_t v_ = 1.l)
	{
		__Assign(0.l);
...
private:
	Tensor2s* __pData()
	{
		return is0() ? (pData = new Tensor2s) : pData;
	}
	bool __AdjustAdd(const Tensor2a& o_)
	{
		return is0() ? (o_.is0() ? false : (pData = new Tensor2s(*o_.pData), false))
					 : (o_.is0() ? false : true);
	}
	template <typename T>
	bool __AdjustMult(const T& o_)
...
public:
	...
	real_t Contraction() const
	{
		return is0() ? 0.l : pData->Contraction();
	}
	real_t Det() const
	{
		return is0() ? 0.l : pData->Det();
	}
	real_t I1() const
	{
		return Contraction();
	...
	{
		return is0() ? 0.l : pData->Minor(i_, j_);
	}
	Tensor2a& Negate()
	{
		return is0() ? *this : (pData->Negate(), *this);
	...
	{
		return is0() ? (o_.is0() ? *this : (pData = new Tensor2s(*o_.pData, false), *this))
	...
	Tensor2a& Assign1(real_t v_ = 1.l)
	{
		if (is0())
			pData = new Tensor2s(eUnit, v_);
		else
			pData->Assign1(v_);
		return *this;
	}
	...
	{
		return is0() || o_.is0() ? 0.l : pData->Dot2Product(*o_.pData);
	}
	...
	Tensor4s& DotProduct(const Tensor4s&, Tensor4s&) const;
	//   Tensor2& ApplyFunction(const fClassicFunction& fun_) {AllocData(0.l);
...
class RetTensor2
{
	...
	RetTensor2(Tensor2a& o_) : pData(o_.pData)
	{
		o_.pData = NULL;
	}
...
public:
	...
	{
		std::for_each(Data, Data + 6, fMilt<real_t, real_t>(v_));
		return *this;
	}
	...
	SymmetricTensor2s& Assign1(real_t v_ = 1.l)
	{
		Data[0] = Data[1] = Data[2] = v_;
		Data[3] = Data[4] = Data[5] = 0.l;
		return *this;
	}
	real_t Contraction() const
	{
...
public:
	Tensor3s(){__SET_ROWS3} Tensor3s(tensor_kind_t, real_t v_ = 1.l)
	{
		__SET_ROWS3;
		Assign1(v_);
	}  // Levy-Chivita
	...
	{
		std::for_each(Data, Data + 27, fMilt<real_t, real_t>(v_));
		return *this;
	}
	Tensor3s& operator/=(real_t);
	Tensor3s& Negate()
	{
		std::transform(Data, Data + 27, Data, std::negate<real_t>());
		return *this;
	}
	bool is0() const
	{
...
public:
	Tensor4s(){__SET_ROWS4} Tensor4s(tensor_kind_t, small_t kind_, real_t v_ = 1.l)
	{
		__SET_ROWS4;
		Assign1(kind_, v_);
	...
	{
		std::for_each(Data, Data + 81, fMilt<real_t, real_t>(v_));
		return *this;
	}
	Tensor4s& operator/=(real_t);
	Tensor4s& Negate()
	{
		std::transform(Data, Data + 81, Data, std::negate<real_t>());
		return *this;
	}
...
#endif
	Data[--number_] = v_;
	Data[number_ == 0 ? 1 : 0] = Data[number_ == 2 ? 1 : 2] = 0.l;
}
//------------------------------------------------------------------------------
...
inline Tensor1s& Tensor2s::AttachedVector(Tensor1s& result_) const
{
	return result_.Assign(
		0.5l * (Data[7] - Data[5]), 0.5l * (Data[2] - Data[6]), 0.5l * (Data[3] - Data[1]));
}
//------------------------------------------------------------------------------
...
#define SQRTL sqrtl
	...
	g[0][1] = l(1, 2) * l(1, 2);
	g[0][2] = l(1, 3) * l(1, 3);
	g[0][3] = 2.l * l(1, 2) * l(1, 3);
	g[0][4] = 2.l * l(1, 3) * l(1, 1);
	g[0][5] = 2.l * l(1, 2) * l(1, 1);
	g[1][0] = l(2, 1) * l(2, 1);
	g[1][1] = l(2, 2) * l(2, 2);
	g[1][2] = l(2, 3) * l(2, 3);
	g[1][3] = 2.l * l(2, 3) * l(2, 2);
	g[1][4] = 2.l * l(2, 3) * l(2, 1);
	g[1][5] = 2.l * l(2, 2) * l(2, 1);
	g[2][0] = l(3, 1) * l(3, 1);
	g[2][1] = l(3, 2) * l(3, 2);
	g[2][2] = l(3, 3) * l(3, 3);
	g[2][3] = 2.l * l(3, 3) * l(3, 2);
	g[2][4] = 2.l * l(3, 3) * l(3, 1);
	g[2][5] = 2.l * l(3, 2) * l(3, 1);
	g[3][0] = l(3, 1) * l(2, 1);
	g[3][1] = l(3, 2) * l(2, 2);
	g[3][2] = l(3, 3) * l(2, 3);
	 ...
	 * g[0][4]=          2.l*l(1,2)*l(1,3); g[0][5]=          2.l*l(1,3)*l(1,1);
	 * g[0][3]=          2.l*l(1,2)*l(1,1);
   g[1][0]=l(2,1)*l(2,1); g[1][1]=l(2,2)*l(2,2);
	 * g[1][2]=l(2,3)*l(2,3); g[1][4]=          2.l*l(2,3)*l(2,2);
	 * g[1][5]=          2.l*l(2,3)*l(2,1); g[1][3]=          2.l*l(2,2)*l(2,1);

	 * g[2][0]=l(3,1)*l(3,1); g[2][1]=l(3,2)*l(3,2); g[2][2]=l(3,3)*l(3,3);
	 * g[2][4]=          2.l*l(3,3)*l(3,2); g[2][5]=          2.l*l(3,3)*l(3,1);
	 * g[2][3]=          2.l*l(3,2)*l(3,1);
