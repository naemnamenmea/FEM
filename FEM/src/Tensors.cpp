...
Tensor2s& Tensor2s::operator-=(const Tensor2s& o_)
{
	...
	for (real_t* pd = Data; pd < Data + 9; ++pd, ++po) *pd -= *po;
	return *this;
}
//------------------------------------------------------------------------------
Tensor2s& Tensor2s::operator+=(const Tensor2s& o_)
...
#endif
	det1 = 1.l / det1;
	real_t tmp[9] = {(Data[4] * Data[8] - Data[5] * Data[7]) * det1,
...
void Tensor2s::EigenValues(real_t eigVals_[3]) const
{
	...
	const real_t i1 = I1(), i2 = I2(), i3 = I3(), i1_3 = i1 / 3.l, p = i2 - i1 * i1_3,
				 q = i1_3 * (i2 - 2.l * i1_3 * i1_3) - i3;	// coefs of reduced eqn
	...
	if (NE0(ro))
	{
		...
		eigVals_[0] = eigVals_[1] = eigVals_[2] =
			(sign(pow(fabs(q), real_t(1.l / 3.l)), -q) + i1_3);
}
//------------------------------------------------------------------------------
struct less_in_abs
{
...
#endif
		eigVects_[j][0] = eigVects_[j][1] = eigVects_[j1][2] = eigVects_[j2][2] = 0.l;
		eigVects_[j][2] = 1.l;
		return 3;
	}
...
{
	const real_t* po = o_.Data;
	for (real_t* pd = Data; pd < Data + 27; ++pd, ++po) *pd -= *po;
	return *this;
}
//------------------------------------------------------------------------------
Tensor3s& Tensor3s::operator+=(const Tensor3s& o_)
...
{
	const real_t* po = o_.Data;
	for (real_t* pd = Data; pd < Data + 81; ++pd, ++po) *pd -= *po;
	return *this;
}
//------------------------------------------------------------------------------
Tensor4s& Tensor4s::operator+=(const Tensor4s& o_)
